library(dplyr)
ownload.file("https://d396qusza40orc.cloudfront.net/getdata%2Fprojectfiles%2FUCI%20HAR%20Dataset.zip",destfile = "./data.zip")
unzip("./data.zip")
...
#the averages of each variables for each activity
apply(data_all4[,1:66],2,function(x) tapply(x,data_all4$activity_label,mean))
#the average of each variables for each subject
